...
import { Account, Role, ShirtSize } from '@app/core/models';
...
export class ShirtSizesAddEditPage implements OnInit, OnDestroy {
	...
	ngOnDestroy(): void {
		console.warn('Add/Edit Shirt Sizes Page ngOnDestroy');
	...
	private uiStateInfo():void {
		...
		.pipe(
			ofType(
				ShirtSizeActions.ShirtSizeActionTypes.MODEL_CreateSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				ShirtSizeActions.ShirtSizeActionTypes.MODEL_UpdateSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				ShirtSizeActions.ShirtSizeActionTypes.MODEL_CreateFailed,
				ShirtSizeActions.ShirtSizeActionTypes.MODEL_UpdateFailed
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
	...
	private createRecord() {
		this.store.dispatch(
			new ShirtSizeActions.MODEL_CreateInitiated({ dataItem: {
				...this.form.value
			}})
		);
	}

	private updateRecord( data: ShirtSize ) {
		this.store.dispatch(
			new ShirtSizeActions.MODEL_UpdateInitiated({ dataItem: {
				...data
				//id: this.id.toString(),
			}})
		);
