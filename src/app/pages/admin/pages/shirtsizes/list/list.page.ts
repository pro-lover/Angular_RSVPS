...
import { Account, ShirtSize } from '@app/core/models';
...
export class ShortSizesListPage implements OnInit, OnDestroy {
	...
	public dataView$!: Observable<ShirtSize[]>;
	...
	constructor(
		...
		this.dataView$ = this.store.select(ShirtSizeSelectors.selectCollection).pipe(takeUntil(this._destroy$));
	}
	...
	private uiStateInfo() {
		...
		.pipe(
			ofType(
				ShirtSizeActions.ShirtSizeActionTypes.MODEL_UpdateStatusSuccess,
				ShirtSizeActions.ShirtSizeActionTypes.MODEL_DeleteSuccess,
				ShirtSizeActions.ShirtSizeActionTypes.MODEL_RestoreSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				ShirtSizeActions.ShirtSizeActionTypes.MODEL_UpdateStatusFailed,
				ShirtSizeActions.ShirtSizeActionTypes.MODEL_DeleteFailed,
				ShirtSizeActions.ShirtSizeActionTypes.MODEL_RestoreFailed
	...
	private initialise( data:ShirtSize[]):void {
	...
	private updateStatus( id: string, params: any ):void {
		...
		this.store.dispatch(
			new ShirtSizeActions.MODEL_UpdateStatusInitiated({
				dataId: id,
				params: params
			})
		);
	...
	public deleteModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new ShirtSizeActions.MODEL_DeleteInitiated({ dataId: id }) );
	...
	public restoreModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new ShirtSizeActions.MODEL_RestoreInitiated({ dataId: id }) );
	...
	public export(): void {
		...
		/* save to file */
		XLSX.writeFile(wb, 'TBWA_Africa_Conference_All_Shirt_Sizes.xlsx');
