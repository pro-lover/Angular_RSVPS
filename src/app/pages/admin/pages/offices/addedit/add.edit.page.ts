...
import { Account, Office, Role } from '@app/core/models';
...
export class OfficesAddEditPage implements OnInit, OnDestroy {
	...
	ngOnDestroy(): void {
		console.warn('Add/Edit Animation Page ngOnDestroy');
	...
	private uiStateInfo():void {
		...
		.pipe(
			ofType(
				OfficeActions.OfficeActionTypes.MODEL_CreateSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				OfficeActions.OfficeActionTypes.MODEL_UpdateSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				OfficeActions.OfficeActionTypes.MODEL_CreateFailed,
				OfficeActions.OfficeActionTypes.MODEL_UpdateFailed
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
	...
	private createRecord() {
		this.store.dispatch(
			new OfficeActions.MODEL_CreateInitiated({ dataItem: {
				...this.form.value
			}})
		);
	}

	private updateRecord( data: Office ) {
		this.store.dispatch(
			new OfficeActions.MODEL_UpdateInitiated({ dataItem: {
				...data
				//id: this.id.toString(),
			}})
		);
