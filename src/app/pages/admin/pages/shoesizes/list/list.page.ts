...
import { Account, ShoeSize } from '@app/core/models';
...
export class ShoeSizesListPage implements OnInit, OnDestroy {
	...
	public dataView$!: Observable<ShoeSize[]>;
	...
	constructor(
		...
		this.dataView$ = this.store.select(ShoeSizeSelectors.selectCollection).pipe(takeUntil(this._destroy$));
	}
	...
	private uiStateInfo() {
		...
		.pipe(
			ofType(
				ShoeSizeActions.ShoeSizeActionTypes.MODEL_UpdateStatusSuccess,
				ShoeSizeActions.ShoeSizeActionTypes.MODEL_DeleteSuccess,
				ShoeSizeActions.ShoeSizeActionTypes.MODEL_RestoreSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				ShoeSizeActions.ShoeSizeActionTypes.MODEL_UpdateStatusFailed,
				ShoeSizeActions.ShoeSizeActionTypes.MODEL_DeleteFailed,
				ShoeSizeActions.ShoeSizeActionTypes.MODEL_RestoreFailed
	...
	private initialise( data:ShoeSize[]):void {
	...
	private updateStatus( id: string, params: any ):void {
		...
		this.store.dispatch(
			new ShoeSizeActions.MODEL_UpdateStatusInitiated({
				dataId: id,
				params: params
			})
		);
	...
	public deleteModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new ShoeSizeActions.MODEL_DeleteInitiated({ dataId: id }) );
	...
	public restoreModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new ShoeSizeActions.MODEL_RestoreInitiated({ dataId: id }) );
	...
	public export(): void {
		...
		/* save to file */
		XLSX.writeFile(wb, 'TBWA_Africa_Conference_All_ShoeSizes.xlsx');
