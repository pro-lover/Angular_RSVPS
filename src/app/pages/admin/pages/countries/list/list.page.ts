...
import { Account, Country } from '@app/core/models';
...
export class CountryListPage implements OnInit, OnDestroy {
	...
	public dataView$!: Observable<Country[]>;
	...
	constructor(
		...
		this.dataView$ = this.store.select(CountrySelectors.selectCollection).pipe(takeUntil(this._destroy$));
	}
	...
	private uiStateInfo() {
		...
		.pipe(
			ofType(
				CountryActions.CountryActionTypes.MODEL_UpdateStatusSuccess,
				CountryActions.CountryActionTypes.MODEL_DeleteSuccess,
				CountryActions.CountryActionTypes.MODEL_RestoreSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				CountryActions.CountryActionTypes.MODEL_UpdateStatusFailed,
				CountryActions.CountryActionTypes.MODEL_DeleteFailed,
				CountryActions.CountryActionTypes.MODEL_RestoreFailed
	...
	private initialise( data:Country[]):void {
	...
	private updateStatus( id: string, params: any ):void {
		...
		this.store.dispatch(
			new CountryActions.MODEL_UpdateStatusInitiated({
				dataId: id,
				params: params
			})
		);
	...
	public deleteModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new CountryActions.MODEL_DeleteInitiated({ dataId: id }) );
	...
	public restoreModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new CountryActions.MODEL_RestoreInitiated({ dataId: id }) );
	...
	public export(): void {
		...
		/* save to file */
		XLSX.writeFile(wb, 'TBWA_Africa_Conference_All_Countries.xlsx');
