...
import { Account, Diet } from '@app/core/models';
...
export class DietListPage implements OnInit, OnDestroy {
	...
	public dataView$!: Observable<Diet[]>;
	...
	constructor(
		...
		this.dataView$ = this.store.select(DietSelectors.selectCollection).pipe(takeUntil(this._destroy$));
	}
	...
	private uiStateInfo() {
		...
		.pipe(
			ofType(
				DietActions.DietActionTypes.MODEL_UpdateStatusSuccess,
				DietActions.DietActionTypes.MODEL_DeleteSuccess,
				DietActions.DietActionTypes.MODEL_RestoreSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				DietActions.DietActionTypes.MODEL_UpdateStatusFailed,
				DietActions.DietActionTypes.MODEL_DeleteFailed,
				DietActions.DietActionTypes.MODEL_RestoreFailed
	...
	private initialise( data:Diet[]):void {
	...
	private updateStatus( id: string, params: any ):void {
		...
		this.store.dispatch(
			new DietActions.MODEL_UpdateStatusInitiated({
				dataId: id,
				params: params
			})
		);
	...
	public deleteModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new DietActions.MODEL_DeleteInitiated({ dataId: id }) );
	...
	public restoreModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new DietActions.MODEL_RestoreInitiated({ dataId: id }) );
	...
	public export(): void {
		...
		/* save to file */
		XLSX.writeFile(wb, 'TBWA_Africa_Conference_All_Diet_Options.xlsx');
