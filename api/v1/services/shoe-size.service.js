...
async function getAll() {
	   ...
	   const models = await db.ShoeSize.findAll({
		paranoid: false,
		order: [
			['name', 'ASC']
		],
...
async function create(params, editId) {
	// validate
	if (await db.ShoeSize.findOne({ where: {
		[Op.or]: [{name: params.name}]
		} }))
	{
		throw 'Shoe Size already exists. Please provide a unique Name.';
	}

    const model = new db.ShoeSize(params);
...
async function update(id, params, editId) {
	...
	if (params.name && model.name == params.name ) {
		throw 'Shoe Size hasn\'t been updated.';
	}
	// validate (if name/shortname is unique in db)
	if ( params.name && (model.name !== params.name) && await db.ShoeSize.findOne({ where: { name: params.name } })) {
		throw 'Shoe Size already exists.';
	}
...
async function getModelById(id) {
    ...
    const model = await db.ShoeSize.findByPk(id, {
		paranoid: false,
		//lock: true,
		//transaction,
		/** /
    ...
    if (!model) throw 'Shoe Size not found';
    return model;
}
