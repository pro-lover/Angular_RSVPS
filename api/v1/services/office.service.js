...
async function getAll() {
    ...
    const models = await db.Office.findAll({
		paranoid: false,
		order: [
			['name', 'ASC']
		],
...
async function create(params, editId) {
	// validate
	if (await db.Office.findOne({ where: {
		[Op.or]: [{name: params.name}]
		} }))
	{
		throw 'Office already exists. Please provide a unique Name.';
	}

    const model = new db.Office(params);
...
async function update(id, params, editId) {
	...
	if (params.name && model.name == params.name ) {
		throw 'Office hasn\'t been updated.';
	}
	// validate (if name/shortname is unique in db)
	if ( params.name && (model.name !== params.name) && await db.Office.findOne({ where: { name: params.name } })) {
		throw 'Office already exists.';
	}
...
async function getModelById(id) {
    ...
    const model = await db.Office.findByPk(id, {
		paranoid: false,
		//lock: true,
		//transaction,
		/** /
    ...
    if (!model) throw 'Office not found';
    return model;
}
