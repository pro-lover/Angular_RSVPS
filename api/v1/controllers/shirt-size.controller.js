...
const shirtsizeService = require(path.join(__dirname, '../services/shirt-size.service'));
...
function getById(req, res, next) {
	...
	if ( Number(req.params.id) ) {
        shirtsizeService.getById(req.params.id)
...
function create(req, res, next) {
	shirtsizeService.create(req.body, req.user.id)
...
function update(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		shirtsizeService.update(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function updateStatus(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		shirtsizeService.updateStatus(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function _delete(req, res, next) {
    ...
    if ( Number(req.params.id) ) {
        shirtsizeService.delete(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function restore(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		shirtsizeService.restore(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
