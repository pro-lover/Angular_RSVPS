...
const shoesizeService = require(path.join(__dirname, '../services/shoe-size.service'));
...
function getById(req, res, next) {
	...
	if ( Number(req.params.id) ) {
        shoesizeService.getById(req.params.id)
...
function create(req, res, next) {
	shoesizeService.create(req.body, req.user.id)
...
function update(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		shoesizeService.update(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function updateStatus(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		shoesizeService.updateStatus(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function _delete(req, res, next) {
    ...
    if ( Number(req.params.id) ) {
        shoesizeService.delete(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function restore(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		shoesizeService.restore(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
