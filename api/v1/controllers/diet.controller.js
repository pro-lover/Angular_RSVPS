...
const dietService = require(path.join(__dirname, '../services/diet.service'));
...
function getById(req, res, next) {
	...
	if ( Number(req.params.id) ) {
        dietService.getById(req.params.id)
...
function create(req, res, next) {
	dietService.create(req.body, req.user.id)
...
function update(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		dietService.update(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function updateStatus(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		dietService.updateStatus(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function _delete(req, res, next) {
    ...
    if ( Number(req.params.id) ) {
        dietService.delete(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function restore(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		dietService.restore(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
